    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.insuranceLoginId) {
            this.$http({
              url: this.$http.adornUrl(`/claim/login/info/${this.dataForm.insuranceLoginId}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/claim/login/${!this.dataForm.insuranceLoginId ? 'save' : 'update'}`),
              method: 'post',
              ...
              if (data && data.code === 0) {
                ...
                if(!this.dataForm.insuranceLoginId?'save' : 'update'=='sava'){
                  this.$message.error("添加失败,信息重复")
                }else{
                  this.$message.error("修改失败,信息重复")
                }
              }
            })
          }
