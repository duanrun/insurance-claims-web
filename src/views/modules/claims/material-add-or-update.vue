<template>
  <el-dialog
    ...
    <el-form-item label="被保险人的id" prop="insuredId">
      <el-input v-model="dataForm.insuredId" placeholder="被保险人的id"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          insuredId: [
            { required: true, message: '被保险人的id不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.materialId) {
            this.$http({
              url: this.$http.adornUrl(`/claim/material/info/${this.dataForm.materialId}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/claim/material/${!this.dataForm.materialId ? 'save' : 'update'}`),
              method: 'post',
